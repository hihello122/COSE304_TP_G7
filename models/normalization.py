...
class MovingBatchNormNd(nn.Module):
    ...
    def forward(self, x, logpx=None):
        ...
        if self.training:
            ...
            # apply lag to batch mean for more stable updates
            if self.bn_lag > 0:
        ...
        # perform normalization (MovingBatchNorm은 평균만 정규화)
        used_mean = used_mean.view(*self.shape).expand_as(x)

        y = x - used_mean # 평균만 빼는 정규화
        ...
        if logpx is None:
            ...
            # MovingBatchNorm의 logdet는 0이므로 logpx는 변경되지 않음
            return y, logpx
